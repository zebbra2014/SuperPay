...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #superpayTEST2\r");
            Send(hSocket, "WHO #superpayTEST2\r");
        } else {
            // randomly join #superpay00-#superpay05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #superpay%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #superpay%02d\r", channel_number).c_str());
        }

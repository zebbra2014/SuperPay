...
static const char *strDNSSeed[][2] = {
    {"superpay.karelia.pro", "dnsseed.superpay.karelia.pro"},
    {"superpay.ru", "dnsseed.superpay.ru"},
    {"superpay.ru", "testseed.superpay.ru"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. SuperPay is probably already running."), addrBind.ToString().c_str());
        else

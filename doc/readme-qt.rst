Superpay-qt: Qt4 GUI for Superpay
===============================
...
Alternatively, install Qt Creator and open the `superpay-qt.pro` file.

An executable named `superpay-qt` will be built.
...
A warning for people using the *static binary* version of Superpay on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Superpay is linked against libdb5.3.
...
installed causes superpay-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
